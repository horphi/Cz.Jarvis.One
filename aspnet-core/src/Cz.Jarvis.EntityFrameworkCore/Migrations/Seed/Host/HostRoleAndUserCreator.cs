...
namespace Cz.Jarvis.Migrations.Seed.Host
{
    public class HostRoleAndUserCreator
    {
        ...
        public HostRoleAndUserCreator(JarvisDbContext context)
        {
            _context = context;
        }
        ...
        private void CreateHostRoleAndUsers()
        {
            ...
            var adminRoleForHost = _context.Roles.IgnoreQueryFilters().FirstOrDefault(r => r.Name == StaticRoleNames.Host.Admin);
            if (adminRoleForHost == null)
            {
                adminRoleForHost = _context.Roles.Add(new Role(StaticRoleNames.Host.Admin, StaticRoleNames.Host.Admin) { IsStatic = true, IsDefault = true }).Entity;
                _context.SaveChanges();
            }

            //admin user for host

            var adminUserForHost = _context.Users.IgnoreQueryFilters().FirstOrDefault(u => u.UserName == AbpUserBase.AdminUserName);
            if (adminUserForHost == null)
            {
                var user = new User
                {
                    ...
                    Name = "admin",
                    Surname = "admin",
                    EmailAddress = "admin@czjarvis.com",
