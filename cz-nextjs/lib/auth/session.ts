 ...
 */
function normalizeRoleClaim(roleClaim: string | string[] | undefined): string[] {
    if (!roleClaim) {
        return [];
    }
 ...
 */
export function extractSessionDataFromToken(token: string, refreshToken: string = ''): AuthSessionData {
    ...
    const roleClaim = decoded["http://schemas.microsoft.com/ws/2008/06/identity/claims/role"] as string | string[] | undefined;
    const normalizedRoles = normalizeRoleClaim(roleClaim);

    // Extract user info from token claims
    return {
        userId: typeof decoded.sub === 'string' ? decoded.sub : (typeof decoded.user_identifier === 'string' ? decoded.user_identifier : ''),
        userName: typeof decoded["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name"] === 'string' ? decoded["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name"] : '',
        ...
        email: typeof decoded["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"] === 'string' ? decoded["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"] : ''
    };
}
